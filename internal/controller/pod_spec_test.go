...
func TestBuildPodSpec_UpstreamExtraCAs_AddsVolumeMountAndArg(t *testing.T) {
	...
	for i := range ps.Volumes {
		if ps.Volumes[i].Name == upstreamExtraCAsVolumeName {
			vol = &ps.Volumes[i]
			break
		}
	}
	if vol == nil {
		t.Fatalf("Volume %q not found in PodSpec.Volumes", upstreamExtraCAsVolumeName)
	}
	if vol.ConfigMap == nil {
		t.Fatalf("Volume %q is not a ConfigMap source: %+v", upstreamExtraCAsVolumeName, vol)
	}
	...
	for _, m := range proxy.VolumeMounts {
		if m.Name == upstreamExtraCAsVolumeName && m.MountPath == upstreamExtraCAsMountPath && m.ReadOnly {
			hasMount = true
			break
		}
	}
	if !hasMount {
		t.Errorf("proxy container missing readOnly mount for %s at %s; mounts=%+v",
			upstreamExtraCAsVolumeName, upstreamExtraCAsMountPath, proxy.VolumeMounts)
	}

	// Arg on proxy container.
	wantArg := fmt.Sprintf("--upstream-ca-bundle=%s/%s", upstreamExtraCAsMountPath, upstreamExtraCAsBundleKey)
	var hasArg bool
	for _, a := range proxy.Args {
		if a == wantArg {
			hasArg = true
...
func TestBuildPodSpec_UpstreamExtraCAs_EmptyOmitsAll(t *testing.T) {
	...
	for _, v := range ps.Volumes {
		if v.Name == upstreamExtraCAsVolumeName {
	...
	for i := range ps.InitContainers {
		...
		for _, m := range c.VolumeMounts {
			if m.Name == upstreamExtraCAsVolumeName {
				t.Errorf("proxy container has %s mount when field is empty", upstreamExtraCAsVolumeName)
			}
		}
