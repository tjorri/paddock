...
- **`transparent` is the default** when the run's namespace admits `NET_ADMIN` on init containers (typically PSA `baseline`, not `restricted`). The iptables-init init container (`cmd/iptables-init/`) runs with `CAP_NET_ADMIN` dropped after exit; the agent container itself remains under `restricted`. The proxy reads `SO_ORIGINAL_DST` to recover the intended destination.
- **`cooperative` is the fallback** when PSA blocks NET_ADMIN. The agent gets `HTTPS_PROXY`, `HTTP_PROXY`, `NO_PROXY=127.0.0.1,localhost,kubernetes.default.svc`, plus CA-trust envs (§7.4 of the spec). The agent must cooperate; this mode is documented as "not sufficient for hostile co-tenant posture — use with a trusted agent image only."
...
- Most operators in hostile-co-tenant settings will either relax PSA on run namespaces (conscious choice, documented) or wait for CNI mode. Homelab operators run on `baseline` and get transparent for free.
