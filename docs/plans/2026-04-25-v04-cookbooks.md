...
**Goal:** Create `docs/cookbooks/` — a permanent operator-facing reference subtree with 9 markdown files (1 index + 8 cookbook pages) — relocate the workflow content currently in the v0.3→v0.4 migration doc into the new cookbook tree, append a v0.4 admission-updates subsection to spec 0002 §2, and ensure cross-references work end-to-end.

**Architecture:** Pure docs work, no code. Three buckets: (1) new cookbook subtree under `docs/cookbooks/`, (2) migration doc shrinks back to upgrade-path content with a 5-line breadcrumb, (3) spec 0002 §2 gains an additive subsection. Every cookbook page follows a uniform 5-section template (summary, when-to-use, setup, worked example, cross-references) so readers learn one rhythm.

**Tech Stack:** Markdown only. No build step, no test infrastructure.

**Related design doc:** `docs/plans/2026-04-25-v04-cookbooks-design.md` (resolved 5 organizational questions).
...
|---|---|---|
| `docs/cookbooks/README.md` | Index + ToC | ~50 |
| `docs/cookbooks/picking-a-delivery-mode.md` | Decision-tree entry point | ~150 |
| `docs/cookbooks/usersuppliedsecret.md` | 4 pattern subsections (header/queryParam/basicAuth/inContainer) | ~300 |
| `docs/cookbooks/anthropic-api.md` | Vertical provider | ~120 |
| `docs/cookbooks/github-app.md` | Vertical provider | ~150 |
| `docs/cookbooks/pat-pool.md` | Vertical provider | ~120 |
| `docs/cookbooks/interception-mode.md` | Relocated from migration doc + intro/cross-refs | ~80 |
| `docs/cookbooks/bootstrapping-an-allowlist.md` | Relocated from migration doc + intro/cross-refs | ~100 |
| `docs/cookbooks/discovery-window.md` | Relocated from migration doc + intro/cross-refs | ~150 |

### Files modified

| Path | Change |
|---|---|
| `docs/migrations/v0.3-to-v0.4.md` | Remove sections "Interception mode", "Bootstrapping an allowlist", "Discovery window" (lines 62–229). Add 5-line "Ongoing operational guidance" breadcrumb at end. Net: ~228 → ~80 lines. |
| `docs/specs/0002-broker-proxy-v0.3.md` | Append `### 2.x Admission updates in v0.4` subsection at the end of §2 (before the `## 3. Scope` header at line 26). ~30 lines added. |

### Files NOT touched

- `docs/adr/0015-provider-interface.md` — out of scope per design doc.
- `docs/specs/0003-broker-secret-injection-v0.4.md` — already documents the design intent; cookbooks reference it.
...
- [Spec 0003](../specs/0003-broker-secret-injection-v0.4.md) — design intent for the v0.4 broker model.
- [ADR-0015](../adr/0015-provider-interface.md) — provider-interface developer reference (where relevant).
```
...
**Cross-link path convention:** within `docs/cookbooks/`, sibling links use `./<file>.md`. Links to `docs/specs/` use `../specs/<file>.md`. Links to `docs/adr/` use `../adr/<file>.md`. Always relative paths — no absolute paths, no GitHub URLs.

**Verification:** for each cookbook, after writing, run `grep -E '\[.*\]\(\./.*\.md\)' <file>` to confirm cross-references match the convention. After all cookbooks land, run a final `find docs/cookbooks/ -name '*.md' -exec grep -L 'picking-a-delivery-mode' {} \;` to confirm every page back-links to the entry point (the README is the only legitimate exception).

---

## Task 1: Scaffold `docs/cookbooks/` + README + decision-tree cookbook

**Files:**
- Create: `docs/cookbooks/README.md`
- Create: `docs/cookbooks/picking-a-delivery-mode.md`
...
Create `docs/cookbooks/picking-a-delivery-mode.md`. The page is the entry point
...
Run: `ls -la docs/cookbooks/`
Expected: `README.md` and `picking-a-delivery-mode.md` present.

Run: `grep -E '\[.*\]\(\./.*\.md\)' docs/cookbooks/picking-a-delivery-mode.md | wc -l`
Expected: at least 4 sibling links present (back to siblings via `./...`).

Run: `wc -l docs/cookbooks/README.md docs/cookbooks/picking-a-delivery-mode.md`
Expected: README ~50 lines, picking-a-delivery-mode ~140-160 lines.

- [ ] **Step 4: Commit**

```bash
git add docs/cookbooks/README.md docs/cookbooks/picking-a-delivery-mode.md
...
**Files:**
- Create: `docs/cookbooks/usersuppliedsecret.md`

- [ ] **Step 1: Write the file**

Create `docs/cookbooks/usersuppliedsecret.md`. The page covers all four
...
Run: `wc -l docs/cookbooks/usersuppliedsecret.md`
Expected: ~280-320 lines.

Run: `grep -c '^## Pattern ' docs/cookbooks/usersuppliedsecret.md`
Expected: 4 (one per pattern subsection).

Run: `grep -E 'kind: UserSuppliedSecret' docs/cookbooks/usersuppliedsecret.md | wc -l`
Expected: 5 occurrences (4 pattern examples + 1 in the worked example).

- [ ] **Step 3: Commit**

```bash
git add docs/cookbooks/usersuppliedsecret.md
...
**Files:**
- Create: `docs/cookbooks/anthropic-api.md`
- Create: `docs/cookbooks/github-app.md`
- Create: `docs/cookbooks/pat-pool.md`
...
Run: `wc -l docs/cookbooks/anthropic-api.md docs/cookbooks/github-app.md docs/cookbooks/pat-pool.md`
Expected: anthropic ~95-115 lines, github-app ~140-160, pat-pool ~115-135.

Run: `for f in docs/cookbooks/{anthropic-api,github-app,pat-pool}.md; do grep -c '^## ' "$f"; done`
...
```bash
git add docs/cookbooks/anthropic-api.md docs/cookbooks/github-app.md docs/cookbooks/pat-pool.md
...
**Files:**
- Create: `docs/cookbooks/interception-mode.md`
- Create: `docs/cookbooks/bootstrapping-an-allowlist.md`
- Create: `docs/cookbooks/discovery-window.md`
...
Open `docs/migrations/v0.3-to-v0.4.md`. Find lines 62–229 (the three
...
how-to lives in [docs/cookbooks/](../cookbooks/README.md). Cookbooks
relevant to the v0.4 changes documented above:

- [picking-a-delivery-mode.md](../cookbooks/picking-a-delivery-mode.md) —
  decision tree for choosing a provider and substitution pattern.
- [usersuppliedsecret.md](../cookbooks/usersuppliedsecret.md) — the v0.4
  successor to the v0.3 `Static` provider.
- [interception-mode.md](../cookbooks/interception-mode.md) —
  transparent vs cooperative interception with the
  `cooperativeAccepted` opt-in.
- [bootstrapping-an-allowlist.md](../cookbooks/bootstrapping-an-allowlist.md) —
  building an egress allowlist for a new harness.
- [discovery-window.md](../cookbooks/discovery-window.md) —
...
Run: `wc -l docs/migrations/v0.3-to-v0.4.md docs/cookbooks/{interception-mode,bootstrapping-an-allowlist,discovery-window}.md`
...
Run: `grep -c 'cookbooks/' docs/migrations/v0.3-to-v0.4.md`
Expected: at least 5 (the breadcrumb section's links).

Run: `grep -L 'picking-a-delivery-mode' docs/cookbooks/{interception-mode,bootstrapping-an-allowlist,discovery-window}.md`
Expected: empty output (all three back-link to the entry point).

- [ ] **Step 6: Commit**

```bash
git add docs/cookbooks/interception-mode.md \
        docs/cookbooks/bootstrapping-an-allowlist.md \
        docs/cookbooks/discovery-window.md \
...
Run: `grep -n '^## 3\. Scope' docs/specs/0002-broker-proxy-v0.3.md`
...
In `docs/specs/0002-broker-proxy-v0.3.md`, just before the line
...
Run: `grep -n '^### 2\.x Admission updates in v0\.4' docs/specs/0002-broker-proxy-v0.3.md`
...
Run: `grep -c 'cookbooks/' docs/specs/0002-broker-proxy-v0.3.md`
Expected: at least 3 (cross-references in the new subsection).

- [ ] **Step 4: Commit**

```bash
git add docs/specs/0002-broker-proxy-v0.3.md
...
**Files:** all of `docs/cookbooks/*.md`, `docs/migrations/v0.3-to-v0.4.md`,
`docs/specs/0002-broker-proxy-v0.3.md`. No expected modifications — this
...
Run: `find docs/cookbooks/ -name '*.md' ! -name 'README.md' ! -name 'picking-a-delivery-mode.md' -exec grep -L 'picking-a-delivery-mode' {} \;`
...
Run: `for f in docs/cookbooks/*.md; do echo -n "$f: "; grep -c '^## See also$' "$f"; done`
...
Run: `for f in $(ls docs/cookbooks/*.md | grep -v README); do basename "$f"; done | sort`
Then: `grep -oE '\[.*\]\(\./.*\.md\)' docs/cookbooks/README.md | sed -E 's/.*\(\.\/(.*)\)/\1/' | sort -u`
...
Run: `for f in docs/cookbooks/*.md docs/migrations/v0.3-to-v0.4.md docs/specs/0002-broker-proxy-v0.3.md; do
...
```bash
git add docs/cookbooks/ docs/migrations/v0.3-to-v0.4.md docs/specs/0002-broker-proxy-v0.3.md
...
|---|---|
| `docs/cookbooks/README.md` index | Task 1 |
